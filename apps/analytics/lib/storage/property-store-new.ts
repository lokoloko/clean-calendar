/**
 * Property Storage Layer - Database Only Implementation
 * Direct database storage without localStorage fallback
 */
...
export class PropertyStore {
   ...
   * Note: This is now async - components need to handle this
   */
  static async getAll(): Promise<Property[]> {
    try {
      ...
      // Get properties with their latest metrics from the view
      ...
      const transformedProperties = (properties || []).map(dbProp => {
  ...
  static async getById(id: string): Promise<Property | null> {
    try {
      ...
      const transformed = this.transformDBToProperty(
        property,
        dataSources || [],
        metrics?.[0]
      )
   ...
   * This is a simplified version - should be optimized for database
   */
  static calculateMetrics(property: Property): PropertyMetrics {
    // Import the calculation logic from the old PropertyStore
    // This is temporary - should be moved to database functions
    const PropertyStoreOld = require('./property-store').PropertyStore
    return PropertyStoreOld.calculateMetrics(property)
  }
  ...
  private static transformDBToProperty(
    dbProp: any,
    ...
    if (metrics || dbProp.revenue) {
      ...
      property.metrics = {
        ...
        health: Math.round((m.confidence || 0) * 0.8)
      }
    }
