...
ADD COLUMN is_active_on_airbnb BOOLEAN DEFAULT true,
...
ADD COLUMN source TEXT DEFAULT 'airbnb' CHECK (source IN ('airbnb', 'manual', 'manual_recurring')),
ADD COLUMN manual_rule_id UUID REFERENCES public.manual_schedule_rules(id) ON DELETE SET NULL;
...
CREATE INDEX idx_manual_schedule_rules_listing_id ON public.manual_schedule_rules(listing_id);
CREATE INDEX idx_manual_schedule_rules_cleaner_id ON public.manual_schedule_rules(cleaner_id);
CREATE INDEX idx_manual_schedule_rules_is_active ON public.manual_schedule_rules(is_active);
CREATE INDEX idx_schedule_items_source ON public.schedule_items(source);
CREATE INDEX idx_schedule_items_manual_rule_id ON public.schedule_items(manual_rule_id);
